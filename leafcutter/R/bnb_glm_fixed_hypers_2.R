...
#' @export
bnb_glm_fixed_hypers_2 <- function(x, cluster_counts, hypers, fit_null=NULL, ...) {
  
  model_to_use=leafcutter:::stanmodels$bnb_glm_fixed_hypers_lib_size
  ...
  dat_null=list(N=nrow(xNull), P=ncol(xNull), y=melted_counts$count, x=xNull, 
                conc=hypers$conc[as.character(melted_counts$intron)], 
                nb_size=hypers$nb_size[as.character(melted_counts$sample)],
                library_size=hypers$library_size[as.character(melted_counts$sample)] )
