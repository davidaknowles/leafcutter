...
#' @export
splicing_qtl_bnb_2=function(counts,geno,geno_meta,fitted_bnb,snps_within=1e4,min_samples_per_intron=5,min_coverage=20,min_samples_per_group=8,timeout=10,debug=F,...) {
  ...
  res=foreach (clu=clusters_to_test, .errorhandling = if (debug) "stop" else "pass") %dopar% {
    ...
    cluster_counts=t(counts[ cluster_ids==clu,, drop=F ])
    ...
    cluster_counts=cluster_counts[,introns_to_use,drop=F]
    ...
    clures=foreach (cis_snp = cis_snps, .errorhandling = if (debug) "stop" else "pass") %do% {
      ...
      if ( sum(introns_to_use)<2 )
        ...
        res <- R.utils::evalWithTimeout( { bnb_glm_fixed_hypers_2(xh,cluster_counts,fitted_bnb,fit_null=NULL,...) }, timeout=timeout, onTimeout="silent" )
