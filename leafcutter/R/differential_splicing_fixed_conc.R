...
#' @export
differential_splicing=function(counts, x, conc, max_cluster_size=10, min_samples_per_intron=5, min_samples_per_group=4, min_coverage=20, timeout=10,  debug=F) {
  ...
  results=foreach (cluster_name=clu_names, .errorhandling = "pass") %dopar% {
    ...
    res <- R.utils::evalWithTimeout( { 
      dirichlet_multinomial_fixed_conc(xFull,xNull,cluster_counts, conc[colnames(cluster_counts)], debug=debug)
