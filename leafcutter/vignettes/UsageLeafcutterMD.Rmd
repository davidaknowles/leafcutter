...
Note, for illustrative purposes (and reduced example file sizes) we use the same data as in the [differential splicing example](./Usage.html), but this data is not a rare disease cohort and thus the final results have little meaning. 
...
We can now use our nice intron count file to do outlier splicing analysis (this assumes you have successfully installed the `leafcutter` R package as described under [Installation](./Installation.html) above)
```
