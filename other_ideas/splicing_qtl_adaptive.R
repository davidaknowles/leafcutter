...
#' @export
splicing_qtl_adaptive=function(counts,geno,geno_meta,pcs=matrix(0,ncol(counts),0),snps_within=1e4,min_samples_per_intron=5,min_coverage=20,min_samples_per_group=8,timeout=10,debug=F,...) {
  ...
  res=foreach (clu=clusters_to_test, .errorhandling = if (debug) "stop" else "pass") %dopar% {
    ...
    clures=foreach (cis_snp = cis_snps, .errorhandling = if (debug) "stop" else "pass") %do% {
      ...
      xFull=cbind(1,pcs_here,xh)
        ...
        res <- R.utils::evalWithTimeout( { dirichlet_multinomial_anova_mc(xFull,xNull,cluster_counts,fit_null=cached_fit_null,robust=F,...) }, timeout=timeout, onTimeout="silent" )
        if (is.null(res)) "timeout" else {
          cached_fit_null=res$fit_null
          if (res$lrtp < .01) {
            res <- R.utils::evalWithTimeout( { dirichlet_multinomial_anova_ht(xFull,xNull,cluster_counts,M=4,robust=T,...) }, timeout=200, onTimeout="silent" )
            if (is.null(res)) res="timeout"
          }
          res
        }
    }
