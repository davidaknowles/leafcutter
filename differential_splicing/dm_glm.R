#require(rstan)
#DIRICHLET_MULTINOMIAL_GLM=stan_model("dm_glm.stan")
...
dirichlet_multinomial_glm <- function(x,y,concShape=1.0001,concRate=1e-4) {
  ...
  o=optimizing(stanmodels$dm_glm, data=dat, as_vector=F)
...
dirichlet_multinomial_anova <- function(xFull,xNull,y,concShape=1.0001,concRate=1e-4) {
  ...
  fit_null=optimizing(stanmodels$dm_glm, data=dat, as_vector=F)
  ...
  # fit model (optimization, don't judge)
  fit_full=optimizing(stanmodels$dm_glm, data=dat_full, init=init, as_vector=F)
